...
const TAMSAMSOMExplorer = () => {
  // Industry presets with default market sizes
  const industryPresets = {
    b2bSaas: {
      name: 'B2B SaaS (Enterprise)',
      globalTAM: 250, // $bn
      avgPrice: 50000, // annual contract
  ...
  // Geographic distribution defaults
  const geoDefaults = {
    northAmerica: 35,
    europe: 28,
    apac: 25,
  ...
  const [viewMode, setViewMode] = useState('topDown'); // 'topDown' or 'bottomUp'
  ...
  // SOM / Venture capacity (for both modes)
  ...
  const [growthRate, setGrowthRate] = useState(80); // % YoY
  ...
  const calculations = useMemo(() => {
    ...
    const impliedMaxShare = Math.min(40, 100 / (competitorCount + 1)); // rough heuristic
    const somYear1TopDown = sam * (yearOneShare / 100);
    
    // SOM projections over time (top-down)
  ...
  const CircleVisualization = () => {
    ...
    return (
      <svg width="320" height="320" style={{ display: 'block', margin: '0 auto' }}>
        ...
        {/* TAM circle */}
        <circle cx="160" cy="160" r={tamRadius} fill="url(#tamGrad)" stroke="#3d5a6a" strokeWidth="2" />
        
        {/* SAM circle */}
        <circle cx="160" cy="160" r={samRadius} fill="url(#samGrad)" stroke="#4a90a4" strokeWidth="2" />
        
        {/* SOM circle */}
        <circle cx="160" cy="160" r={Math.max(somRadius, 8)} fill="url(#somGrad)" stroke="#6bb8c9" strokeWidth="2" filter="url(#glow)" />
        
        {/* Labels */}
        <text x="160" y="35" textAnchor="middle" fill="#6b7b8a" fontSize="11" fontWeight="500">TAM</text>
        <text x="160" y="50" textAnchor="middle" fill="#8aa0b0" fontSize="13" fontFamily="'JetBrains Mono', monospace">{formatBn(calculations.tam)}</text>
        
        {samRadius > 40 && (
          <>
            <text x="160" y={160 - samRadius + 20} textAnchor="middle" fill="#7b9bab" fontSize="10" fontWeight="500">SAM</text>
            <text x="160" y={160 - samRadius + 34} textAnchor="middle" fill="#9bbaca" fontSize="12" fontFamily="'JetBrains Mono', monospace">{formatBn(calculations.sam)}</text>
          </>
        )}
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '32px', display: 'flex', alignItems: 'center', gap: '20px' }}>
          <div className="mode-toggle">
            <button 
              className={`mode-btn ${viewMode === 'topDown' ? 'active' : ''}`}
              onClick={() => setViewMode('topDown')}
            >
              Top-Down
            </button>
            <button 
              className={`mode-btn ${viewMode === 'bottomUp' ? 'active' : ''}`}
          ...
          <span style={{ fontSize: '12px', color: '#5a6a7a' }}>
            {viewMode === 'topDown' 
              ? 'Start with total market, apply filters' 
              : 'Start with unit economics, build upward'}
          </span>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr 320px', gap: '32px' }}>
          ...
          {/* Center Panel - Visualization & Waterfall */}
          <div>
            {/* Key Metrics */}
            <div style={{ 
              display: 'grid', 
              ...
              {viewMode === 'topDown' ? (
                <>
                      ...
                      <div className="projection-bar-container">
                        <div 
                          ...
                          style={{ 
                            ...
                            background: proj.capped 
                              ? 'linear-gradient(90deg, #8ba090, #6b8070)' 
                              : 'linear-gradient(90deg, #4a90a4, #6bb8c9)'
                          }}
                        />
                      </div>
                      <span className="projection-value">
                        {formatBn(proj.som)}
                        <span style={{ color: '#5a6a7a', fontSize: '10px', marginLeft: '4px' }}>
                          ({proj.share.toFixed(1)}%)
                        </span>
                      </span>
                    </div>
                  ))}
                  ...
                  <div style={{ 
                    marginTop: '20px', 
                    padding: '12px', 
                    background: 'rgba(107, 184, 201, 0.06)', 
                    borderRadius: '6px',
                    fontSize: '11px',
                    color: '#6b7b8a',
                    lineHeight: 1.6
                    ...
                    {calculations.somProjectionsTopDown.some(p => p.capped) && 
                      <span style={{ color: '#8ba090' }}> Ceiling reached.</span>
                    }
                  </div>
                </>
              ) : (
                  ...
                  <div style={{ marginTop: '20px' }}>
                    ...
                    {calculations.bottomUpProjections.map((proj) => (
                      <div key={proj.year} style={{ 
                        ...
                        fontSize: '11px',
                        padding: '6px 0',
                        borderBottom: '1px solid rgba(107, 184, 201, 0.06)'
                      }}>
                        <span style={{ color: '#5a6a7a' }}>Y{proj.year}</span>
                        <span style={{ color: '#7b9bab' }}>{proj.reps} reps</span>
                        <span style={{ color: '#6bb8c9', fontFamily: "'JetBrains Mono', monospace" }}>
                          {proj.deals} deals
                        </span>
                      </div>
                    ))}
